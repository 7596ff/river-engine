...
- [ ] flash subsystem — witness gains a per-turn flash pass replacing the standalone connect duty. Five types ship (Connection, Echo, Return, Correction, Danger); two stubbed pending their signal specs (Bridge — needs shape substrate, now ready; Friction — needs stance scan). Absorbs connect_for + ConnectFrame + on-connect.md + connect-log.jsonl. Spec: `docs/superpowers/specs/2026-07-11-flash-subsystem-design.md`.
- [ ] Bridge wiring — light up the Bridge flash type (text-sim low ∧ shape-sim high). Depends on flash subsystem existing. Uses the shape-index substrate already shipped: gloss_turn composes the turn's shape, Memory::search_shapes retrieves candidates, text-sim filter drops the text-close matches, per-target refractory. Frame body carries both glosses (turn + target) so the signal is legible. Spec §5 of `docs/superpowers/specs/2026-07-12-shape-index-design.md`.
