...
- [ ] migration: `embeddings/atomic` → `knowledge` — 345 pi-era atomics with richer link vocabulary (`responds-to`, `resonates-with`, `parallels`, `contradicts`, etc.) in an older format (extra frontmatter fields: `type`, `created`, `author`; links point to file paths or descriptive IDs rather than ULIDs/timestamp-z). Need a script to convert to current `knowledge/` format, strip obsolete fields, and optionally relink where targets resolve. Blocked on: deciding whether to preserve dead links or attempt resolution.
